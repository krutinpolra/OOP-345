...
namespace seneca {
   ...
   size_t Utilities::getFieldWidth() const
   {
	...
	std::string Utilities::trim(std::string str, const std::string space)
	{
		size_t strtingOfString = str.find_first_not_of(space);

		if (strtingOfString == std::string::npos)
			return "";

		size_t endingOfString = str.find_last_not_of(space);
		size_t lengthOfString = endingOfString - strtingOfString + 1;

		return (str = str.substr(strtingOfString, lengthOfString));
	}
